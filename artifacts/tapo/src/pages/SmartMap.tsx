...
export default function SmartMap() {
  ...
  const insight = useMemo(() => {
    ...
    // Breakdown kategori sesuai legenda tagging yang sedang dipilih
    const kategoriCount: Record<string, number> = {};

    filteredHouses.forEach((h) => {
      let kategori = "Tidak Ada/Lainnya";

      if (taggingMode === "luasBangunan") {
        const v = Number(h.luasBangunan);
        if (v > 100) kategori = "> 100 m²";
        else if (v >= 50) kategori = "50 – 100 m²";
        else if (v > 0) kategori = "< 50 m²";
      } else if (taggingMode === "luasLahan") {
        const v = Number(h.luasLahan);
        if (v > 200) kategori = "> 200 m²";
        else if (v >= 100) kategori = "100 – 200 m²";
        else if (v > 0) kategori = "< 100 m²";
      } else if (taggingMode === "jumlahLantai") {
        const v = Number(h.jumlahLantai);
        if (v === 1) kategori = "Lantai 1";
        else if (v === 2) kategori = "Lantai 2";
        else if (v >= 3) kategori = "Lantai 3+";
      } else if (taggingMode === "jenisDinding") {
        const v = String(h.jenisDinding ?? "").toLowerCase();
        if (v.includes("tembok")) kategori = "Tembok";
        else if (v.includes("kayu")) kategori = "Kayu";
        else if (v.includes("bambu") || v.includes("seng")) kategori = "Bambu/Seng";
      } else if (taggingMode === "jenisPlafon") {
        const v = String(h.jenisPlafon ?? "").toLowerCase();
        if (v.includes("triplek") || v.includes("asbes") || v.includes("bambu")) {
          kategori = "Triplek/Asbes/Bambu";
        } else if (v.includes("pvc")) {
          kategori = "PVC";
        } else if (v.includes("beton") || v.includes("plat")) {
          kategori = "Beton/Plat";
        } else if (
          v.includes("kayu") ||
          v.includes("akustik") ||
          ...
          kategori = "Kayu/Gypsum/Kalsibor";
        }
      } else if (taggingMode === "jeniLantai") {
        const v = String(h.jeniLantai ?? "").toLowerCase();
        if (v === "keramik") kategori = "Keramik";
        else if (v.includes("marmer") || v.includes("granit")) kategori = "Marmer/Granit";
        else if (v === "semen") kategori = "Semen";
        else if (v.includes("kayu") || v.includes("papan")) kategori = "Kayu/Papan";
      }
    ...
    const breakdown = (mode?.legend ?? []).map((item) => {
      const n = kategoriCount[item.label] ?? 0;
